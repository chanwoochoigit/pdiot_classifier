...
def run_conv_model(X_train, X_test, y_train, y_test):
    ...
    print(X_test.shape)
    model = Sequential()
    model.add(Conv1D(filters=filter_size, kernel_size=kernel_size, input_shape=(X_train.shape[1], X_train.shape[2]), padding='same'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))

    model.add(Conv1D(filters=filter_size, kernel_size=kernel_size, padding='same'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    ...
    model.add(Dense(1155, kernel_initializer='he_uniform'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
